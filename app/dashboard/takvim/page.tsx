...
const BEYANNAME_COLOR: Record<string, { bg: string; color: string; border: string }> = {
  kdv:       { bg: "#eff6ff", color: "#1d4ed8", border: "#bfdbfe" },
  muhtasar:  { bg: "#f5f3ff", color: "#6d28d9", border: "#ddd6fe" },
  sgk_prim:  { bg: "#fff7ed", color: "#c2410c", border: "#fed7aa" },
  ba_bs:     { bg: "#f0f9ff", color: "#0369a1", border: "#bae6fd" },
  gecici:    { bg: "#f0fdf4", color: "#15803d", border: "#bbf7d0" },
  yillik_gv: { bg: "#fefce8", color: "#a16207", border: "#fef08a" },
  kurumlar:  { bg: "#fef2f2", color: "#dc2626", border: "#fecaca" },
...
export default async function TakvimPage() {
  ...
  const t = getDict(locale).takvim;
  const dateFnsLocale = locale === "tr" ? tr : enUS;

  const now  = new Date();
  const year = now.getFullYear();
  const allItems = generateTaxCalendar(year, locale);
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div>
        <h1 className="text-xl font-semibold tracking-tight" style={{ color: "var(--text-1)" }}>{t.pageTitle}</h1>
        <p className="text-[13px] mt-0.5" style={{ color: "var(--text-3)" }}>
          {t.filingDeadlinesSub.replace("{year}", String(year))}
        </p>
      </div>
      ...
      <div className="rounded-xl overflow-hidden"
        ...
        <div className="grid grid-cols-3">
          {[
            { label: t.thisMonth,  value: thisMonth.length, numColor: "#2563eb",  bg: "transparent" },
            { label: t.next60Days, value: upcoming.length, numColor: "#d97706",  bg: "transparent" },
            { label: t.pastDue,    value: overdue.length,   numColor: overdue.length > 0 ? "#dc2626" : "var(--text-3)", bg: overdue.length > 0 ? "var(--red-bg)" : "transparent" },
      ...
      <div className="rounded-xl overflow-hidden"
        ...
        <div className="px-5 py-3.5" style={{ borderBottom: "1px solid var(--border-2)" }}>
          <h2 className="text-[13px] font-semibold" style={{ color: "var(--text-1)" }}>{t.upcomingFilings60}</h2>
        </div>
        {upcoming.length === 0 ? (
          <p className="px-5 py-10 text-center text-[13px]" style={{ color: "var(--text-3)" }}>
            {t.noFilingsNext60Days}
          </p>
        ) : (
          <div>
            {upcoming.map((item, i) => {
              ...
              const cfg = BEYANNAME_COLOR[item.code] ?? BEYANNAME_COLOR.default;
              return (
                <div key={i} className="px-5 py-3 flex items-center justify-between"
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[13px] tabular-nums" style={{ color: "var(--text-2)" }}>
                      {format(due, "d MMMM yyyy", { locale: dateFnsLocale })}
                    </span>
      ...
      <div className="rounded-xl overflow-hidden"
        ...
        <div className="px-5 py-3.5" style={{ borderBottom: "1px solid var(--border-2)" }}>
          <h2 className="text-[13px] font-semibold" style={{ color: "var(--text-1)" }}>
            {t.monthFilings.replace("{month}", format(now, "MMMM yyyy", { locale: dateFnsLocale }))}
          </h2>
        </div>
        <div>
            ...
            return (
              <div key={i} className="px-5 py-3 flex items-center justify-between"
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[13px] tabular-nums" style={{ color: "var(--text-2)" }}>
                    {format(due, "d MMMM", { locale: dateFnsLocale })}
                  </span>
                  {passed && (
